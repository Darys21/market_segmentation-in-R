...
#Les numéros de cluster correspondant aux différents segments sont affichés avec la fonction table() ci-dessus. Par exemple, si le résultat est :
 # 1 2 3 4
#35 50 22 48
#cela signifie qu'il y a 35 observations dans le segment 1, 50 dans le segment 2, 22 dans le segment 3 et 48 dans le segment 4.
...
#Ces analyses  permettent de mieux comprendre les comportements d'achat des clients dans chaque région et dans chaque segment de marché, ce qui  aidera à concevoir des stratégies de marketing et des programmes promotionnels adaptés aux préférences des consommateurs.
...
# graphe qui montrera le nombre de produit vendus par segment dans chaque région
ggplot(store_qty, aes(x = region, y = qty_sum, fill = segment)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Nombre de produits vendus par segment dans chaque région", 
       x = "Région", y = "Nombre de produits vendus") +
